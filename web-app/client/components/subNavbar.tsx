    ...
    <NextUINavbar maxWidth="xl" position="sticky">
      <NavbarContent className="basis-1/5 sm:basis-full" justify="start">
        <NavbarBrand as="li" className="gap-3 max-w-fit">
          ...
          <NextLink className="flex justify-start items-center gap-1" href="/market" passHref>
            <Link>
              <p className="font-bold text-inherit">Market Data</p>
            </Link>
          </NextLink>
        </NavbarBrand>
        <ul className="hidden lg:flex gap-4 justify-start ml-2">
          {marketConfig.navItems.map((item) => (
      ...
      <NavbarContent className="md:hidden basis-1 pl-4" justify="end">
        <Link isExternal aria-label="Github" href={marketConfig.links.github}>
      ...
      <NavbarMenu>
        ...
        <div className="mx-4 mt-2 flex flex-col gap-2">
          {marketConfig.navItems.map((item, index) => (
              ...
              {hasDropdown(item) ? (
                ...
                <Link
                  color={
                    index === 2
                      ? "primary"
                      : index === marketConfig.navItems.length - 1
                        ? "danger"
                        : "foreground"
                  }
                  href={item.href}
